...
% Modifies load in the full DBS model
...
f=6.78e6;		%Frecuency
T=1/f;			%Period
periods=100;	%Integration Periods	

%% Allow Iterations
R0=9.13;
iterate=true;	%Iterate values
RES=10;         %Iteration Number
if iterate==true
    RR=linspace(1,40,RES);
else
    RR=0;
end
...
%% Iterations
% The script iterates the simulation for every RR element
for cycles=1:1:length(RR)
    
    text = sprintf('R0: %f', RR(cycles));	
    waitbar(cycles/length(RR),wait,text);					%Visualization of the iterations
	LTmodify( file_name, 'Load', num2str(RR(cycles)));		%Modify Load with RR
    tic;													%Start measuring time
    raw=LTautomation(file_name);							%Run the LT simulation
    toc;													%Show elapsed time
	sim_length=length(raw.time_vect);
		
	%% Integration
	% Only the last periods will be analyzed
...
%% End the simulations
% Proces the retrieved data, put the initial values in the simulation
LTmodify( file_name, 'Load', '{Rout}');
delete(wait)
...
hold on
grid on;
plot(LT_Poutv,LT_efic_total,'LineWidth',linewidth);
plot(LT_Poutv,LT_efic_inverter,'LineWidth',linewidth);
plot(LT_Poutv,LT_efic_link,'LineWidth',linewidth);
plot(LT_Poutv,LT_efic_aux,'LineWidth',linewidth);
plot(LT_Poutv,LT_efic_rect,'LineWidth',linewidth);
...
grid on;
yyaxis left 
plot(RR(1:length(LT_Poutv)),LT_efic_total,'--b','LineWidth',linewidth);
xlabel('R0(\Omega)')
ylabel('\eta')
yyaxis right 
plot(RR(1:length(LT_Poutv)),LT_Poutv,'--r','LineWidth',linewidth);
xlabel('R_l_o_a_d (\Omega)')
ylabel('P_o_u_t [W]')
title('\eta vs Pout')

save('../../data/LTsim_PowervsLoad.mat')
